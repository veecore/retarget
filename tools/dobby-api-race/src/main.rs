...
/// One successfully resolved case paired with its baseline address.
#[derive(Clone, Debug)]
struct Baseline {
...
/// Resolves one candidate through the `dobby-api` wrapper.
fn resolve(candidate: Candidate) -> Option<usize> {
    match candidate.image {
        Some(image) => dobby_api::resolve_func_addr(Some(image), candidate.symbol).ok(),
        None => dobby_api::resolve_func_addr::<&str>(None, candidate.symbol).ok(),
    }5
...
fn probe() -> Vec<Baseline> {
    candidates()
        ...
        .filter_map(|candidate| {
            resolve(candidate).map(|address| Baseline { candidate, address })
        })
        .collect()
}
    ...
    for worker_index in 0..threads {
        ...
        workers.push(thread::spawn(move || {
            ...
            for iteration in 0..iterations {
                ...
                match resolve(baseline.candidate) {
...
fn main() -> ExitCode {
    ...
    println!(
        "dobby-api race probe on {} with {threads} threads, {iterations} iterations, {rounds} rounds",
        env::consts::OS,
    );
